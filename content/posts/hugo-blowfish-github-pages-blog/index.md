...
| 继续用 Pages 的地址，但**构建前统一规范成 https** | **只改 Pages 设置**，代码不用动 | 自适应；地址永远跟着域名走，代价是构建时多一行处理 |
...
**① 构建期断言**：构建完成后、上传产物之前，直接检查**刚生成的首页里引用的样式表路径，是否与本次构建用的地址一致**。这一步纯本地、不碰网络：路径对不上，就让这次运行直接失败。这样一来，"地址不一致导致没样式"会变成 CI 上的红叉，而不是等我偶然发现。

这一条其实是我改过的**第二版**。最初写的是"**部署后自检**"——部署完成后去请求线上首页和它引用的样式表，任一不是 200 就判失败。它有两个坑，最后被我整个删掉了：

- **必须带重试**：我第一版只检查一次，结果部署刚结束、CDN 还没切到新内容，它抓到的是**上一版** HTML，于是误报失败——站点其实已经好了；
