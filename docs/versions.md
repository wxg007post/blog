...
|---|---|---|
| #4 | 12 秒 | 第 1 次检查即崩 |
| #5 | **1 分 4 秒** | 进了重试循环（约 4 次 15 秒睡眠）后中途崩 |
| #6 | 9 秒 | 第 1 次检查即崩 |
| #7（已加 `::error::` 诊断） | 9 秒 | **注解里没有我的诊断文字** → 证明脚本根本没走到那一步 |

**根因**：GitHub Actions 的 `run:` 默认以 **`bash -e`** 执行（任何命令返回非零立即终止），脚本里又加了 `set -o pipefail`，而 `grep … | head | sed | tr` 这条管道与 `curl` 有未兜底的调用——一旦返回非零（例如抓回来的不是正常首页、grep 匹配不到 CSS 链接），脚本**当场死掉**，重试与诊断代码都没机会执行。
**修复**：脚本首行 `set +e`，判定规则改为"只在真正该拦的时候拦"（首页 200 + 样式表 404/410 才判红，其余一律绿 + `::warning::`）。

**三条教训（写给未来的自己）**：

1. 在 CI 脚本里，**"自己的错误处理"必须先于"环境异常处理"**——否则后者永远不会生效；
2. **"预期会打印的诊断没打印"本身就是证据**：说明脚本比你以为的更早就死了；
3. 本环境限制：**agent 读不到 GitHub 的运行日志与 API**（注解正文是前端渲染，HTML 里没有；API 匿名限流在这条共享出口 IP 上长期为 0）→ 需要诊断时，请把运行页面上注解的文字**复制给 agent**。
